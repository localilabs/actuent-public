...
import { embedAndSaveSite } from "./embed"
...
const contentForEmbed = Object.values(site.pages).map((p: any) => `${p.title} ${p.content}`).join(" ")
await embedAndSaveSite(site.domain, contentForEmbed)
